...
**Status:** owner pre-read and guided-session worksheet. EP-D01 and EP-D07
closed through explicitly accepted dispositions on 2026-09-03; EP-D02–EP-D06
and EP-A01–EP-A03 remain pending. This packet is not the authoritative
[decision record](endless-progression-decisions.md), a balance specification,
or implementation authorization.

## Session protocol and evidence

1. Answer the three short cluster cards below.
2. The design agent expands the selected labels into complete record text.
...
### EP-A03 fixed invariants for any acceptable model

- Completion creates no licence, unit, item, chassis, currency, Rule Load,
  transferable value, or higher active-copy cap. [A]
- Licences identify exact versioned behavior/burden projections, never chassis
  stats. They are personal and cannot transfer through trade, locker, controller,
  inheritance, salvage, Forge, or record outcomes.
- One personal Reconstruction Tray has four Load-weighted units earned when
  permanent Rule Load first crosses 1, 2, 3, and 4. [A/D]

```text
U_earned = min(maxPermanentlyUnlockedRuleLoad, 4)
U_earned = U_dormant + U_free + U_reserved
          + sum(loadCost(all attached reconstruction projections))
delta U_earned = count({1,2,3,4}
                       intersect (oldPermanentLoad, newPermanentLoad])
  only on an authorized unlock/import/migration receipt
delta U_earned at completion, trade, claim, salvage, reconstruction, or replay = 0
```

All terms are nonnegative integers. Reserve/commit/release is atomic and one
operation ID cannot reserve or refund units twice. Fresh initialization derives
the same `U_earned` total directly; it does not invent historical receipts.

- A projection attaches between Circuits to one personally owned,
  behavior-empty, eligible, exact-compatible host that is outside battle,
  trade, escrow, Forge, salvage, retirement, and locker transitions. One host
  carries at most one projection. Dismantling restores the same host and only
  refunds its units.
- Attachment changes no chassis fact or physical item count. A reconstructed
  projection still counts as one copy for all combatant/team copy and stacking
  caps. Active physical plus reconstructed copies may not exceed the authored
  cap.
- The guaranteed baseline host multiset must admit a matching for every
  otherwise legal combination totaling at most four Load. Failure leaves this
  specification blocked. [U]
- Completion evaluates every potentially legal ID as licensed/deepened or
  irreversibly retired. Prune, an unresolved offer, or avoiding a source does
  not count.
- Retirement is legal only at an Epoch boundary after the personal source is
  cleared and three distinct persisted offer receipts show that exact ID was
  offered and declined. At most one ID retires per boundary; baseline and
  mandatory IDs are ineligible. It is also refused while the ID appears in an
  unresolved offer, owned unsunk carrier, reconstruction, loadout, plan, or
  snapshot. It creates no replacement, currency, unit, reroll, or source credit.

**Seam:** Endless sidecar, inventory, and designed-rule projection only.

**Invited degeneration:** perfect counter wardrobe, trade teaching permanent
rights, chassis laundering, virtual-copy stacking, or rushing the completion
marker.

**Counter:** personal earning receipts, exact hosts, unit/copy equations,
between-Circuit locks, finite attunement, one bounded Pivot, fault tests, and
zero-currency Keep/Salvage/retirement policy comparisons.

### Licence event

Choose one:

- **L1 — both earned cache candidates:** paid settlement licences both exact
  behavior candidates, even if one is later declined or salvaged. Prepared,
  unearned, Standard/no-behavior, trade, and borrowed items create none. This
  makes Keep and all-Salvage histories converge faster but accelerates library
  breadth. An unresolved offer never counts merely because it exists; under L1
  its eligible candidates count only because settlement already wrote their
  durable licences.
- **L2 — one nominated candidate (recommended):** resolving one earned offer
  creates at most one licence. Keep nominates the kept candidate; whole-offer
  Salvage must nominate exactly one eligible behavior candidate in the same
  signed atomic operation. The other remains unresolved. View, inspection,
  disconnect, or an unresolved offer creates none. Duplicate nomination creates
  no second licence. If no eligible behavior candidate exists, Salvage proceeds
  normally and creates no licence.

Under L2, unresolved eligible IDs enter deterministic coverage. No eligible ID
may repeat until every other source-cleared, unlicensed, unretired ID in its
coverage pool has been reoffered; target/duplicate history cannot mark it
resolved. This finite cycle replaces an unbounded random completion tail. [A]

Selected personal Mastery/source receipts licence only their exact projection.
Participation, ally clears, unselected choices, and post-completion records do
not.

### Tray activation

Choose one; neither is a silent default:

- **M1 — at first legitimate completion:** the receipt moves already earned
  units from `U_dormant` to `U_free`. This intentionally creates a lateral
  access transition while preserving scalar, Load, and copy ceilings. Because
  retirement can trigger completion, the owner also accepts that surrendering
  the retired ID may accelerate maintenance. Reject if retirement-rush or
  near-completion liquidation becomes a dominant policy.
- **M2 — progressively with each unit:** each Load-unlock receipt creates a
  usable unit immediately. Completion changes no access. Reject if nomination
  plus Salvage plus free reconstruction makes Keep/Forge broadly dominated or
  collapses the pre-completion acquisition loop.

The transaction that resolves or retires the final outstanding ID atomically
commits the earning/retirement and coverage mutation, completion reevaluation,
M1 `U_dormant -> U_free` movement when selected, completion marker, and
grandfather references, or none of them.

Exact entitlement recycling is not a standalone option: an all-Salvage,
zero-currency history can finish with no tokens while a hoarder retains many.
Requiring items/currency at completion only moves the hoard checkpoint. [D]

### Catalog reopen after maintenance has activated

- **C1 — immediate after a new personal licence (recommended):** existing
  assignments remain; a new ID is reconstructible only after its own earning
  receipt. This avoids a second completion access spike but weakens future
  expansion acquisition.
- **C2 — only after recompletion:** preserves a longer expansion acquisition
  loop but creates another lateral access transition and rush incentive. It
  withholds only IDs added by the reopened definition version. Existing
  assignments and reassignment among pre-reopen licensed IDs remain usable
...
EP-A03 licence: L1 both | L2 one nominated | replace: ...
EP-A03 activation: M1 completion transition | M2 progressive | replace: ...
EP-A03 catalog reopen: C1 immediate after earning | C2 recompletion | replace: ...
EP-A03 fixed invariants: accept | replace: ...
```

These labels select direction only. The complete nomination, activation, host,
copy, retirement, conservation, and fault rules will be replayed before EP-A03
...
- Recovery Rematches restore that encounter/recipe/branch root exactly and
  ...
  grants no mechanical rewards. Reject if forecast-informed Concede dominates
  paid continuation in progression per action or route policy.
