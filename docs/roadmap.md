...
| campaign CIRCUITS — consecutive bouts, survivors carried | `src/campaign/circuit.js`, `tools/hotseat.mjs --circuit` | **landed 2026-09-07, and it is the read-back's first consumer.** `advanceCircuit(battle, { blueprints, challengers, sides })` takes a settled bout to the next one's teams, refilling the eliminated side with a fresh challenger. Two things it REPORTS rather than decides: `restoredResources`, because a record carries no resources, so a survivor rebuilt from his blueprint gets his **armour and stamina back** — measured, a fighter ending at `armourclass 0` re-enters at `495` — and `facingCorrections`, because `facing-left` is a carried status token and a fighter moved across the arena would otherwise keep the facing of the side he left. The exact between-bout restoration policy remains an open progression/balance specification. **There is no default length** — four fights is EP-D03 and EP-D03 is `pending`, so `circuitLength()` refuses an absent value and names the decision |
| campaign rewards | `src/team/ss2-crowd.js` (the build's purse, no caller) | ~~still not started~~ **not PAID anywhere, and deliberately — but no longer "not started" (corrected 2026-09-24).** The BUILD's own victory purse is built and map-derived since `cefaf83` (2026-09-23): `ss2VictoryPurse` is `round(loser character_xp × (100 + crowd_interest) / 100)` added to the winner's gold, except that a level-1 winner's balance is SET to 2,500 (the emperor's gift), and `ss2TeamVictoryPurses` is the owner-decided team generalisation (each winner paid on an EQUAL SHARE of the losing side's `character_xp`, HANDOFF.md decision (f)). Neither has a caller outside `test/ss2-crowd.test.js`: no host, no circuit and no campaign record pays or stores gold. Paying a reward is a progression decision, and the accepted EP decisions live on the design track with ~~EP-D04~~ EP-D05 (personal precommitted reward outcomes; EP-D04 is rarity) pending. A seam that healed or paid the survivors would be a balance choice wearing the costume of a data structure |
| six-slot arena layout, clips, panels | `src/adapter/slot-layout.js`, `src/adapter/presentation.js` | derived asset-free and tested for 1v1/2v2/3v3/1v3, and emitted as JSON presentation commands. **`src/render/` renders them as of 2026-09-10** — the stream's first consumer outside its own tests. Everything past slot 0 of each side is still an authored mod surface no capture can settle, and drawing it surfaced a tension nobody had noticed: the authored ally band sits at a SMALLER y (further back) and a HIGHER depth (drawn in front), which inverts vanilla's own shadow-behind-fighter convention. Pinned in `test/render-scene.test.js`, not silently corrected |
| distinct-human admission and pause/reconnect transport | — | not started; required by accepted EP-D07 before playable 2v2 (Stage 4b). The generic controller and AI-fill seams do not satisfy this product policy |
| broader host-authoritative online transport | — | not started (Stage 7) |
| a rendered arena | `src/render/`, `tools/arena/`, `tools/arena-server.mjs` | **landed 2026-09-10.** ~~Original vector art drawn from code~~ Original vector art drawn from code where no usable extracted pack is present, and the player's OWN extracted art and sound, pack by pack, where one is (corrected 2026-09-24; see the Stage 5 row); no SS2 asset ships, and `test/no-shipped-assets.test.js` now enforces that for the whole repository rather than leaving it to care. It is the first host to set `awaitAnimations: true`, so the per-action animation gate ENFORCES for the first time, and the first thing anywhere to decide the animation-timeout policy that `src/adapter/action-gate.js` deliberately refuses to own |
