...
| 4b. Human session lifecycle | Distinct-human allied-seat admission, command authority, durable action-boundary pause, authenticated same-seat reconnect, and established abandonment flow | required by accepted Endless EP-D07 before a playable team proof; not started. Local multi-input versus remote transport and abandonment liveness remain open. Generic controller reassignment is not a substitute, and allied AI may not finish the MVP battle |
...
- A minimum genuine multi-human session, action-boundary pause, and reconnect
  ...
  stage. Broader online lobby/matchmaking remains later and is not a different
  ruleset.
