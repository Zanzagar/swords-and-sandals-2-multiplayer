...
| a rendered arena | `src/render/`, `tools/arena/`, `tools/arena-server.mjs` | **landed 2026-09-10.** Original vector art drawn from code; no SS2 asset ships, and `test/no-shipped-assets.test.js` now enforces that for the whole repository rather than leaving it to care. It is the first host to set `awaitAnimations: true`, so the per-action animation gate ENFORCES for the first time, and the first thing anywhere to decide the animation-timeout policy that `src/adapter/action-gate.js` deliberately refuses to own |
